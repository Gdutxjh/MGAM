...
class BC(nn.Module):
    ...
    def __init__(self, __C, atten=False):
        ...
        self.v_net = MLP([__C.IMG_FEAT_SIZE,
                          __C.BA_HIDDEN_SIZE], dropout_r=__C.BAN_DROPOUT_R)
    ...
    def forward(self, v, q):
        ...
        v_ = self.dropout(self.v_net(v))    # 8*100*3072
        q_ = self.q_net(q)                  # 8*14*3072
        logits = torch.einsum('xhyk,bvk,bqk->bhvq',
                              (self.h_mat, v_, q_)) + self.h_bias
        return logits  # b x h_out x v x q  # 8*4*100*14
...
class BAN(nn.Module):
    def __init__(self, __C):
        ...
        for i in range(__C.GLIMPSE):
            b_net.append(BC(__C))
            q_prj.append(MLP([__C.BAN_HIDDEN_SIZE, __C.BAN_HIDDEN_SIZE], '', __C.BAN_DROPOUT_R))
