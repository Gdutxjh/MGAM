...
class MFB(nn.Module):
    def __init__(self, config, img_feat_size, ques_feat_size, is_first=True):
        super(MFB, self).__init__()
        self.config = config
        self.is_first = is_first
        self.proj_i = nn.Linear(img_feat_size, self.config.MFB_K * self.config.MFB_O)
        self.proj_q = nn.Linear(ques_feat_size, self.config.MFB_K * self.config.MFB_O)
        self.dropout = nn.Dropout(self.config.MFB_DROPOUT_R)
        self.pool = nn.AvgPool1d(self.config.MFB_K, stride=self.config.MFB_K)
    ...
    def forward(self, img_feat, ques_feat, exp_in=1):
        ...
        exp_out = ques_feat * img_feat
        exp_out = self.dropout(exp_out) if self.is_first else self.dropout(exp_out * exp_in)     # (N, C, K*O)
        z = self.pool(exp_out) * self.config.MFB_K + 1e-6        # (N, C, O)
        z = torch.sqrt(F.relu(z) + 1e-6) - torch.sqrt(F.relu(-z) + 1e-6)
...
class QAtt(nn.Module):
    def __init__(self, __C):
        ...
        self.mlp = MLP(
            ...
            mid_size=__C.HIDDEN_SIZE_MFH,
            out_size=__C.Q_GLIMPSES,
            dropout_r=__C.MFB_DROPOUT_R,
            use_relu=True
        )
